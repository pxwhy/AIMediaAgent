...
import { autoPublishTask, cancelPublishTask, createDraftFromRawContent, createPublishTask, deletePublishTask, loadPublishTaskDiagnostics, markPublishTaskFailed, markPublishTaskPublished, openPublishEditor } from '../../api/client';
export function createPublisherContext(deps) {
    ...
    async function autoPublish(task) {
        ...
        if (!window.confirm(`确认要自动发布「${draftTitle(task.draft_id)}」到${platformName}吗？\n\n系统会自动点击发布按钮，请确认标题、正文和图片已经检查无误。`)) {
            return;
        }
        publisherBusy.value = true;
