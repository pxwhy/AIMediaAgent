...
import { deleteContentSelection, loadAccountProfileReports, loadAccountReviewReports, loadContentSelections, selectCollectedContent } from '../../api/client';
export function createSmartFilterContext(deps) {
    ...
    const contentSelectionHistory = ref([]);
    const contentSelectionProfileReports = ref([]);
    const contentSelectionReviewReports = ref([]);
    const contentSelectionResultFilter = ref('all');
    ...
    const contentSelectionAgents = computed(() => deps.agentConfigs.value.filter((agent) => agent.agent_type === 'content_selection' && agent.enabled));
    const selectedContentSelectionProfile = computed(() => contentSelectionProfileReports.value.find((report) => report.id === contentSelectionProfileReportId.value) ?? null);
    const selectedContentSelectionReview = computed(() => contentSelectionReviewReports.value.find((report) => report.id === contentSelectionReviewReportId.value) ?? null);
    const canRunContentSelection = computed(() => deps.selectedRawContentIds.value.length > 0 || Boolean(selectedContentSelectionProfile.value || selectedContentSelectionReview.value));
    ...
    const contentSelectionRecommendedCount = computed(() => contentSelectionResult.value?.results.filter((item) => item.selected).length ?? 0);
    ...
    function selectContentSelectionRun(run) {
    ...
    async function removeContentSelectionRun(run) {
        if (!run.id) {
            return;
        }
    ...
    function matchCollectorTargetsByProfile(profile) {
    ...
    function matchCollectorTargetsByKeywords(keywords) {
    ...
    function filterExistingRawContentsByTargets(targets) {
        return deps.dashboard.rawContents.filter((content) => targets.some((target) => content.source === target.source && content.category === target.category));
    }
    ...
    function profilePriorityScore(priority) {
        if (priority === 'high')
            return 12;
        if (priority === 'medium')
            return 8;
