...
import type { PaginationKey } from './context'

type PaginationState = Record<PaginationKey, { page: number; pageSize: number }>

export function createPaginationHelpers(pagination: PaginationState) {
...
export function readableError(error: unknown, fallback = '操作失败，请稍后重试') {
...
export function metricText(metrics: Record<string, unknown>, key: string) {
